...
import base64, datetime, difflib, filecmp, hashlib, json, mimetypes, pathlib, platform, re, shutil, signal, ssl, string, subprocess, tempfile, zipfile, zipp
...
class AppManagerServer(object):
        ...
        for srcDir, dirs, files in os.walk(topSrcDir):
            ...
            for file_ in files:
                ...
                for pattern in ignoredPatterns:
                    if pathlib.Path(dstFile).match(pattern):
                        ignore = True
                        break
                if ignore:
                    continue
    ...
    class CreateAppZipHandler(tornado.web.RequestHandler, ResponseTemplates):
        ...
        async def get(self):
            ...
            with tempfile.TemporaryDirectory() as tmpDir:
                ...
                server.copyTreeMerge(appDir, tmpDir)
    ...
    class DropAppZipHandler(tornado.web.RequestHandler, ResponseTemplates):
        ...
        async def post(self):
            ...
            # only one root directory allowed inside the zip
            ...
            self.write(self.renderTemplate('dialog_drop_zip_done.tpl', {
                'nameDisp': appName.replace('_', ' ').title(),
                ...
                if appTemplate == 'cordova':
                    ...
                    if targetPlatform != 'none':
                        ...
                        if targetPlatform in ['darwin-x64', 'darwin-arm64', 'mas-x64', 'mas-arm64']:
                            ...
                            # need four version numbers, the missing ones will be added by peresed
                            peVersion = re.sub('[^\d\.,]', '', appVersion)
                    ...
                    server.replaceTemplateStrings(tplDest / 'main.js', {
                        'mainHTML': mainHTML,
                        'showFullscreen': self.get_argument('showFullscreen', default='false')
                    })
